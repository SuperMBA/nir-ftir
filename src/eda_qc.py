...
Функционал:
- Загрузка train/external parquet, подготовленных prepare_data.py.
- Welch t-test по каждому спектральному признаку + BH-FDR + Cohen's d.
- Оценка ROC-AUC для разных нормализаций (group-aware CV по ID).
- Анализ повторных измерений (реплик) и их стабильности.
- Поиск робастных выбросов (SNV -> PCA(whiten) -> MinCovDet, χ²-порог).
- Анализ метаданных/конфаундеров + baseline AUC по одним метаданным.
- Генерация отчёта (README.md, summary.txt, таблицы и графики)
  в каталоге reports/eda_qc/latest.

Запуск:
    python src/eda_qc.py
    # или с указанием каталога:
    python src/eda_qc.py --outdir reports/eda_qc/latest
"""
...
def pick_spectral_columns(df: pd.DataFrame) -> List[str]:
    """Вернуть список столбцов-волновых чисел, отсортированный по возрастанию."""
    spec_cols = [c for c in df.columns if _is_number_like(c)]
    return sorted(spec_cols, key=lambda c: float(c))


def split_columns(df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    """Разделить столбцы на спектральные и метаданные.

    Возвращает:
        spec_cols: список спектральных столбцов (волновые числа).
        meta_cols: остальные (метаданные, кроме служебных y/Label/ID).
    """
...
def load_processed() -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Загрузить parquet, подготовленные prepare_data.py."""
...
def plot_qvalues(df_p: pd.DataFrame, path: Path) -> None:
    """График -log10(q) по волновым числам для валидных признаков."""
...
def plot_norm_auc(df_norm: pd.DataFrame, path: Path) -> None:
    """Столбчатый график ROC-AUC по схемам нормализации."""
...
def replicate_distances(
    ...
    if "ID" in df_ext.columns:
        ids = df_ext["ID"].astype(str).to_numpy()
    else:
        ids = df_ext.index.astype(str).to_numpy()
    ...
    for uid in sorted(np.unique(ids)):
        ...
        for i in range(idx.size):
            for j in range(i + 1, idx.size):
                dist = np.linalg.norm(arr[idx[i]] - arr[idx[j]])
                rows.append(
                    {
                        "ID": uid,
                        "pair": f"{i + 1}-{j + 1}",
...
def replicate_summary(df_pairs: pd.DataFrame) -> pd.DataFrame:
    """Сводка по каждому ID: n_reps, mean/median/std/cv/max, худшая пара."""
    if df_pairs.empty:
        return pd.DataFrame(
            columns=[
                "ID",
    ...
    grouped = df_pairs.groupby("ID")["dist"]
    ...
    summary = pd.DataFrame(
        {
            ...
            "mean_dist": grouped.mean().values,
            "median_dist": grouped.median().values,
            "std_dist": grouped.std(ddof=1).fillna(0.0).values,
            "max_dist": grouped.max().values,
        }
    )
...
def plot_replicate_hist(
    df_pairs: pd.DataFrame,
    path: Path,
    source_name: str,
) -> None:
    """Гистограмма расстояний между репликами."""
    plt.figure(figsize=(6, 3))
    if len(df_pairs):
...
def analyze_metadata(
    ...
    Эффект:
      - numeric: стандарт. разница средних (standardized mean diff).
      - categorical: Cramer's V (с коррекцией Бенакри–Грина).
    """
    if not meta_cols:
        empty = pd.DataFrame(columns=["feature", "type", "test", "pval", "qval", "effect"])
        return empty, None
...
def write_report_md(
    ...
    lines.append(f"- total features: **{n_total}**, " f"valid: **{n_valid}**, q<0.05: **{n_sig}**")
    ...
    lines.append(f"- Table: [ttest_curve.csv](" f"{(out_dir / 'ttest_curve.csv').as_posix()})")
    lines.append("")
    ...
    if len(df_norm):
        ...
        lines.append(
            f"- Figure: [qc_06_norm_auc.png](" f"{(fig / 'qc_06_norm_auc.png').as_posix()})"
        )
        lines.append(f"- Table: [norm_auc.csv](" f"{(out_dir / 'norm_auc.csv').as_posix()})")
    else:
        lines.append("- no CV results available")
    lines.append("")

    # Replicates
    lines.append(f"## Replicates ({rep_source_name})")
    if df_pairs.empty or df_rep.empty:
        lines.append("- no replicate pairs found")
    else:
        med_mean = df_rep["mean_dist"].median()
        if "cv_dist" in df_rep.columns:
            med_cv = df_rep["cv_dist"].median()
        else:
            med_cv = float("nan")
        lines.append(
            "- IDs with replicates: "
        ...
        lines.append(
            "- Figure: [replicate histogram](" f"{(fig / 'qc_07_replicate_dist.png').as_posix()})"
        )
        lines.append(
        ...
        lines.append(
            "- Per-ID summary: [replicates_summary.csv]("
            f"{(out_dir / 'replicates_summary.csv').as_posix()})"
        )
    lines.append("")
    ...
    if len(df_out):
        cutoff = float(df_out["cutoff_chi2"].iloc[0]) if "cutoff_chi2" in df_out.columns else None
        df_chi = int(df_out["df"].iloc[0]) if "df" in df_out.columns else None
        ...
        for row in df_out.itertuples(index=False):
            rid = f" (ID={row.ID})" if "ID" in df_out.columns else ""
            mark = ""
            if "is_outlier" in df_out.columns and row.is_outlier:
                mark = " *"
            top_inds.append(f"{row.index}{rid}{mark}")
        lines.append("- top indices: " + ", ".join(top_inds))
        lines.append(
            "- Table: [outliers_train.csv](" f"{(out_dir / 'outliers_train.csv').as_posix()})"
        )
    else:
    ...
    if len(df_meta):
        ...
        if auc_meta is not None:
            lines.append(
                "- baseline AUC using *only* metadata: " f"**{auc_meta:.3f}** (group-aware CV)"
            )
        lines.append(
            "- Table: [metadata_summary.csv](" f"{(out_dir / 'metadata_summary.csv').as_posix()})"
        )
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default=str(ROOT / "reports" / "eda_qc" / "latest"),
        help=("Папка для артефактов EDA/QC. " "По умолчанию: reports/eda_qc/latest"),
    )
    ...
    # 3) Реплики: train_repl приоритетнее, иначе external
    rep_source_name = "none"
    df_pairs = pd.DataFrame()
    df_rep = pd.DataFrame()
    rep_source = None

    train_repl_path = DATA_PROCESSED / "train_repl.parquet"
    if train_repl_path.exists():
        df_train_repl = pd.read_parquet(train_repl_path)
        if "ID" in df_train_repl.columns and df_train_repl["ID"].duplicated().any():
            rep_source = df_train_repl
            rep_source_name = "train_repl"

    if rep_source is None and "ID" in df_ext.columns and df_ext["ID"].duplicated().any():
        rep_source = df_ext
        rep_source_name = "external"

    if rep_source is not None:
        spec_cols_rep, _ = split_columns(rep_source)
        df_pairs = replicate_distances(rep_source, spec_cols_rep)
        df_pairs.to_csv(out_dir / "replicate_distances.csv", index=False)
        plot_replicate_hist(
            df_pairs,
            fig_dir / "qc_07_replicate_dist.png",
            rep_source_name,
        )
        df_rep = replicate_summary(df_pairs)
        df_rep.to_csv(out_dir / "replicates_summary.csv", index=False)
    else:
        plot_replicate_hist(
            df_pairs,
            fig_dir / "qc_07_replicate_dist.png",
    ...
    if len(df_meta):
        df_meta.sort_values("qval").to_csv(
    ...
    summary_lines.append(f"train shape: {df_train.shape}, external shape: {df_ext.shape}")
    summary_lines.append(f"n wavenumbers: {X.shape[1]}")
    summary_lines.append(f"ttest: total={n_total}, valid={n_valid}, q<0.05={n_sig}")
    if len(df_norm):
        ranking = ", ".join(f"{row.scheme}={row.roc_auc:.3f}" for row in df_norm.itertuples())
        summary_lines.append(f"AUC by normalization: {ranking}")
    ...
    if len(df_out):
        ...
        if "ID" in df_out.columns:
            ...
            sid = ""
        summary_lines.append(f"top outlier: idx={best['index']}{sid}, dist2={best['dist2']:.3f}")
    if auc_meta is not None:
        summary_lines.append(f"metadata-only AUC: {auc_meta:.3f}")
    ...
    write_report_md(
        ...
        df_meta=df_meta,
        auc_meta=auc_meta,
        rep_source_name=rep_source_name,
    )
